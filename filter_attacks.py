...
def iou_duplicate_filter(df_raw, filtered_df_csv):
    """Filter csv files: 
...
def main(exp_dir, exp_id, name, stop_nemotron):
    ...
    filtered_df_csv = folder_path / 'fixed_filtered_df_old.csv'
    if not filtered_df_csv.exists():
        df = iou_duplicate_filter(df_raw, filtered_df_csv)
    df = pd.read_csv(filtered_df_csv, index_col=False)
    ...
    nemotron_scores_csv = folder_path / 'fixed_nemotron_scores_old.csv'
    ...
    best_flter_csv = folder_path / 'fixed_best_filter_old.csv'
