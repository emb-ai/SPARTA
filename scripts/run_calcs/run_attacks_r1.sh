...
run_attack() {
    ...
    CUDA_VISIBLE_DEVICES=0,1 torchrun --nproc_per_node=2 --master_port=25901 attack_main.py attacker=$attacker dataset=$dataset segmod@_global_=$segmod -o $path -e $segmod max_dataset_len=$max_dataset_len
    wait
}
