...
def nemotron_scoring(df, nemotron_csv):
    ...
    MAIN_PROMPT = """
    You are an AI assistant who will help me to evaluate the quality of the following paraphrase on a scale from 1 to 5 based on its accuracy in preserving the original meaning for the task of referring segmentation. The paraphrase should be grammatically correct. To mark a response, you should output a single integer between 1 and 5 (including 1, 5).
    ...
    5 means the paraphrase fully retains the original sentence's meaning without losing important information or altering any key details and does not violate any grammar rules.
    1 means the paraphrase significantly distorts the meaning by inverting important parts of the original sentence or is grammatically incorrect.
    ...
    Example 4:
    Original: <text>In the Caribbean Sea region, pirate attacks are common, so every ship needs to take appropriate defensive measures. What in the picture can be used to protect the ship from pirate attacks?</text>
    Paraphrase: <text>In Caribbean Sea region, pirate attack are commons, so every ship need takes appropriate defensive measures. What in picture can be use to protected the ship from pirate attack?</text>
    Your mark: 1

    Example 5:
    Original: <text>If we need to enter a room, what is the object we need to go through to enter it?</text>
    Paraphrase: <text>If we needing enter a room, what is object we needing go through for enter it?</text>
    Your mark: 2

    Example 6:
    Original: <text>If I need to print a document, what object in this room will help me complete the task?</text>
    Paraphrase: <text>If I need print a document, what object in this room will helps me complete the task?</text>
    Your mark: 3
    ...
    Example 8:
    Original: <text>What part of the sink can control the water flow?</text>
    Paraphrase: <text>What part of sink can controls the water flow?</text>
    Your mark: 4
...
def cosine_scoring(df, cosine_threshold=0):
    ...
    def cosine_similarity(text1: str, text2: str) -> float:
        ...
        emb1, emb2 = model.encode([text1, text2], convert_to_tensor=True, device=device, normalize_embeddings=True)
        return float((emb1 @ emb2.T).item())

    df['mpnet_cosine'] = None
    df['mpnet_cosine'] = df.apply(lambda x: cosine_similarity(x['orig_text'], x['adv_text']), axis=1)

    df.loc[df['mpnet_cosine'] < cosine_threshold, 'SCORE_Qwen/Qwen3-32B'] = 0
    return df


def fix_register_ending(df, filtered_scores_csv):
    start_upper = df['orig_text'].str.extract('([A-Za-z])', expand=False).str.isupper()
    adv_start_upper = df['adv_text'].str.extract('([A-Za-z])', expand=False).str.isupper()
    cond1_bad = start_upper.fillna(False) ^ adv_start_upper.fillna(False)     

    end_punct = df['orig_text'].str.rstrip().str.endswith(tuple(',.?!'))
    adv_end_punct = df['adv_text'].str.rstrip().str.endswith(tuple(',.?!'))

    cond2_bad = end_punct ^ adv_end_punct                                     
    df.loc[(cond1_bad | cond2_bad), 'SCORE_Qwen/Qwen3-32B'] = 0

    df.to_csv(filtered_scores_csv, index=False)
    return df
...
def best_filter(df, best_flter_csv):
    ...
    df_sorted = df.sort_values(
        by=['sample_idx', 'SCORE_' + MODEL_NAME, 'iou_drop'], ascending=[True, False, False])
...
@click.option('--cosine-threshold', default=0.0, type=float, show_default=True, help='Cosine similarity threshold for filtering')
def main(exp_dir, exp_id, name, stop_nemotron, cosine_threshold):
    ...
    if stop_nemotron:
        df.to_csv(f'new_data_without_nemotron{exp_id}.csv', index=False)
        return
    ...
    filtered_scores_csv = folder_path / 'fixed_nemotron_scores_filtered.csv'
    if not filtered_scores_csv.exists():
        with torch.no_grad():
            # if cosine_threshold > 0:
            #     df = cosine_scoring(df, cosine_threshold)
            df = fix_register_ending(
                df,
                filtered_scores_csv)
    df = pd.read_csv(filtered_scores_csv, index_col=False)
    ...
    if cosine_threshold > 0:
        cos_filtered_csv = folder_path / f'fixed_best_filter_cos={cosine_threshold}.csv'
        if not cos_filtered_csv.exists():
            df = cosine_scoring(df, cos_filtered_csv, cosine_threshold)
        df = pd.read_csv(cos_filtered_csv, index_col=False)
