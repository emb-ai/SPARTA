...
def cosine_scoring(df, cos_filtered_csv, cosine_threshold=0):
    device = 'cuda' if torch.cuda.is_available() else 'cpu'

    from sentence_transformers import SentenceTransformer
    model = SentenceTransformer("Qwen/Qwen3-Embedding-8B", device=device)
    encode_fn = lambda texts: model.encode(texts, convert_to_tensor=True, device=device, normalize_embeddings=True)
    ...
    df.loc[df['qwen_cosine'] < cosine_threshold, 'SCORE_Qwen/Qwen3-32B'] = 0
    df.to_csv(cos_filtered_csv, index=False)
    return df
...
@click.option('-cos', '--cosine-threshold', default=0.0, type=float, show_default=True, help='Cosine similarity threshold for filtering')
def main(exp_dir, exp_id, name, stop_nemotron, cosine_threshold):
    ...
    if cosine_threshold != 0:
        cos_filtered_csv = folder_path / f'fixed_best_filter_qwen3_cos={cosine_threshold}.csv'
