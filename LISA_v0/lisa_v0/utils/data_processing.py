...
def get_mask_from_rle_json(json_file):
    is_sentence = True
    comments = []
    rle_masks = []

    for qa in json_file['qa_pairs']:
        comments.append(qa['question'])
        rle = qa['rle_seg']
    ...
    for rle in rle_masks:
        ...
        mask = mask_utils.decode(rle_formatted)  # shape: (h, w), dtype=uint8
        decoded_masks.append(mask)

    # Step 3: Stack into (n, h, w)
    masks = np.stack(decoded_masks, axis=0)
