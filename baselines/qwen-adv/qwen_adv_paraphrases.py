...
MAIN_PROMPT = """
You are tasked with subtly altering a query for a reasoning segmentation system. The system is designed to output a segmentation mask based on the query text and an image. Your goal is to slightly modify the query so that:
1. The text remains grammatically correct and plausible.
2. The modified query refers to the same general concept or object but introduces subtle ambiguities or misleading details that can cause the system to generate an incorrect segmentation mask.
3. The meaning is implicit and still related to the original context but exploits the reasoning weakness of the system.
Here is the original query:
{orig}
Please provide a subtly modified query that meets the above conditions.
...
@click.option(
    ...
    type=str,
    default=None,
    help="Path to the output JSON file. Defaults to qwen3_adv_paraphrases_<dataset>.json",
)
def cli(dataset: str, num_samples: int | None, output: str | None):
    ...
    if output is None:
        output = f"qwen3_adv_paraphrases_{dataset}.json"
