...
from gsva.model.llava import conversation as conversation_lib
from gsva.model.llava.constants import DEFAULT_IMAGE_TOKEN, IGNORE_INDEX, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
from gsva.model.llava.mm_utils import tokenizer_image_token
from gsva.model.segment_anything import ResizeLongestSide

from .data_processing import get_mask_from_json, get_mask_from_rle_json
...
class ValDataset(TorchDataset):
    ...
    def __init__(
        ...
        if len(splits) == 2:
            ...
            if ds == "grefcoco":
                refer_api = G_REFER(self.base_image_dir, ds, splitBy)
            elif ds == 'refzom':
                refer_api = REFZOM_REFER(self.base_image_dir, 'refer_seg', ds)
            else:
            ...
            ds, split = 'LLMSeg40k', 'val'
            json_path = 'dataset/llm-seg40k/validation.json'
            with open(json_path, 'r') as f:
                self.json_file = json.load(f)
            images = [
                f'dataset/llm-seg40k/train2017/{img_name}'
                for img_name, info in self.json_file.items()
            ]
            self.images = images
            self.data_type = 'llm-seg40k'
    ...
    def __getitem__(self, idx):
        ...
        elif self.data_type == 'llm-seg40k':
            image_path = self.images[idx]
            image = cv2.imread(image_path)
            image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
            img_name = image_path.split('/')[-1]
            mask_json, sampled_sents, is_sentence = get_mask_from_rle_json(self.json_file[img_name])
            sampled_sents = [sampled_sents[0]]
        else:
            raise ValueError(f"Invalid dataset type: {self.data_type}")
